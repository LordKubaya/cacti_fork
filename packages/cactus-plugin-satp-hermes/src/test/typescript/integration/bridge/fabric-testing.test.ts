...
describe("Fabric Bridge Test", () => {
  ...
  it("Wrap asset", async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      log.info("Wrap Wrapped " + i);
    }
  });

  it("Wrap Mint", async () => {
    for (let i = 0; i < 50; i++) {
      const responseMint = await fabricBridge.mintAsset(FABRIC_ASSET_ID, 500);
  ...
  it("Mint ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20MintTime.csv",
        "Duration (ms)\n",
        "" + (endTime - startTime),
      );
      log.info("Mint ERC20 " + i);
    }
  });
  ...
  it("Lock Token", async () => {
    ...
    for (let i = 0; i < 50; i++) {
      const responseLock = await fabricBridge.lockAsset(FABRIC_ASSET_ID, 50);
  ...
  it("Lock ERC20", async () => {
    ...
    for (let i = 0; i < 50; i++) {
      const startTime = new Date().getTime();
      const responseLock = await apiClient.runTransactionV1({
        contractName: satpContractName,
        channelName: fabricChannelName,
        params: [clientId, bridgeId, "5"],
      ...
      const endTime = new Date().getTime();
      expect(responseLock).not.toBeUndefined();
      expect(responseLock.status).toBeGreaterThan(199);
      expect(responseLock.status).toBeLessThan(300);
      expect(responseLock.data).not.toBeUndefined();
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20LockTime.csv",
        "Duration (ms)\n",
        "" + (endTime - startTime),
      );
      log.info("Lock ERC20 " + i);
    }
  });

  it("Burn Wrap", async () => {
    for (let i = 0; i < 50; i++) {
      const responseBurn = await fabricBridge.burnAsset(FABRIC_ASSET_ID, 1);
  ...
  it("Burn ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      const startTime = new Date().getTime();
      const responseBurn = await apiClient.runTransactionV1({
        contractName: satpContractName,
        channelName: fabricChannelName,
        params: ["1"],
        methodName: "burn",
      ...
      const endTime = new Date().getTime();
      expect(responseBurn).not.toBeUndefined();
      expect(responseBurn.status).toBeGreaterThan(199);
      expect(responseBurn.status).toBeLessThan(300);
      expect(responseBurn.data).not.toBeUndefined();
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20BurnTime.csv",
        "Duration (ms)\n",
        "" + (endTime - startTime),
      );
      log.info("Burn ERC20 " + i);
    }
  });

  it("Wrap Unlock", async () => {
    for (let i = 0; i < 50; i++) {
      const responseUnlock = await fabricBridge.unlockAsset(FABRIC_ASSET_ID, 1);
      ...
      log.info("Wrap Unlocked " + i);
    }
  });

  it("Unlock ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      const startTime = new Date().getTime();
      const responseUnlock = await apiClient.runTransactionV1({
        contractName: satpContractName,
        channelName: fabricChannelName,
        params: [bridgeId, "1"],
        methodName: "transfer",
      ...
      const endTime = new Date().getTime();
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20UnlockTime.csv",
        "Duration (ms)\n",
        "" + (endTime - startTime),
      );

      expect(responseUnlock).not.toBeUndefined();
      expect(responseUnlock.status).toBeGreaterThan(199);
      expect(responseUnlock.status).toBeLessThan(300);
      expect(responseUnlock.data).not.toBeUndefined();

      log.info("Unlock ERC20 " + i);
    }
  });

  it("Assign Wrap", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      log.info("Wrap Assigned " + i);
    }
  });

  it("Assign ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      const startTime = new Date().getTime();
      const responseAssign = await apiClient.runTransactionV1({
        contractName: satpContractName,
        channelName: fabricChannelName,
        params: [clientId, bridgeId, "1"],
        methodName: "assign",
        invocationType: FabricContractInvocationType.Send,
        signingCredential: fabricSigningCredential,
      });
      const endTime = new Date().getTime();
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20AssignTime.csv",
        "Duration (ms)\n",
        "" + (endTime - startTime),
      );
      expect(responseAssign).not.toBeUndefined();
      expect(responseAssign.status).toBeGreaterThan(199);
      expect(responseAssign.status).toBeLessThan(300);
      expect(responseAssign.data).not.toBeUndefined();
      log.info("Assign ERC20 " + i);
    }
  });
  ...
  it("Should Unwrap a token", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      const responseUnwrap = await fabricBridge.unwrapAsset(tokenId);
