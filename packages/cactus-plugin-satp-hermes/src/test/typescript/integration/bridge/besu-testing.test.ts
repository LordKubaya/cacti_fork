...
describe("Besu Bridge Test", () => {
  ...
  it("Should Wrap a token", async () => {
    ...
    const response = await besuBridge.wrapAsset(assetIni);
  ...
  it("Wrap Mint", async () => {
    for (let i = 0; i < 50; i++) {
      const response = await besuBridge.mintAsset(BESU_ASSET_ID, 500);
      expect(response).toBeDefined();
      expect(response.transactionId).toBeDefined();
      expect(response.transactionReceipt).toBeDefined();
  ...
  it("Mint ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      const response = (await testing_connector.invokeContract({
        contractName: erc20TokenContract,
        keychainId: keychainPlugin1.getKeychainId(),
        invocationType: EthContractInvocationType.Send,
        methodName: "mint",
        params: [firstHighNetWorthAccount, "300"],
        signingCredential: {
          ethAccount: firstHighNetWorthAccount,
          secret: besuKeyPair.privateKey,
          type: Web3SigningCredentialType.PrivateKeyHex,
      ...
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20MintGasUsed.csv",
        "Gas (units)\n",
  ...
  it("Lock Token", async () => {
    const responseApprove = await testing_connector.invokeContract({
      ...
      invocationType: EthContractInvocationType.Send,
      methodName: "approve",
      params: [wrapperContractAddress, "5000"],
      signingCredential: {
        ethAccount: firstHighNetWorthAccount,
        secret: besuKeyPair.privateKey,
        type: Web3SigningCredentialType.PrivateKeyHex,
  ...
  it("Lock ERC20", async () => {
    const responseApprove = await testing_connector.invokeContract({
      ...
      invocationType: EthContractInvocationType.Send,
      methodName: "approve",
      params: [firstHighNetWorthAccount, "5000"],
      signingCredential: {
        ethAccount: firstHighNetWorthAccount,
        secret: besuKeyPair.privateKey,
        type: Web3SigningCredentialType.PrivateKeyHex,
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const response = (await testing_connector.invokeContract({
        ...
        invocationType: EthContractInvocationType.Send,
        methodName: "transfer",
        params: [firstHighNetWorthAccount, wrapperContractAddress, "1"],
        signingCredential: {
          ethAccount: firstHighNetWorthAccount,
          secret: besuKeyPair.privateKey,
          type: Web3SigningCredentialType.PrivateKeyHex,
      ...
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20LockGasUsed.csv",
        "Gas (units)\n",
        "" + response.out.transactionReceipt.gasUsed,
      );
      expect(response).toBeTruthy();
      expect(response.success).toBeTruthy();
      log.info("Locked ERC20 " + i);
    }
  });

  it("Burn Wrap", async () => {
    for (let i = 0; i < 50; i++) {
      const response = await besuBridge.burnAsset(BESU_ASSET_ID, 1);
      expect(response).toBeDefined();
      expect(response.transactionId).toBeDefined();
      expect(response.transactionReceipt).toBeDefined();
      log.info("Burned " + i);
  ...
  it("Burn ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      const response = (await testing_connector.invokeContract({
        ...
        invocationType: EthContractInvocationType.Send,
        methodName: "burn",
        params: [wrapperContractAddress, "1"],
        signingCredential: {
          ethAccount: firstHighNetWorthAccount,
          secret: besuKeyPair.privateKey,
          type: Web3SigningCredentialType.PrivateKeyHex,
      ...
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20BurnGasUsed.csv",
        "Gas (units)\n",
        "" + response.out.transactionReceipt.gasUsed,
      );
      expect(response).toBeTruthy();
      expect(response.success).toBeTruthy();
      log.info("Burned ERC20 " + i);
    }
  });

  it("Unlock a token", async () => {
    for (let i = 0; i < 50; i++) {
      const response = await besuBridge.unlockAsset(BESU_ASSET_ID, 1);
      expect(response).toBeDefined();
      expect(response.transactionId).toBeDefined();
      expect(response.transactionReceipt).toBeDefined();
      log.info("Unlocked " + i);
  ...
  it("Unlock ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      const response = (await testing_connector.invokeContract({
        ...
        invocationType: EthContractInvocationType.Send,
        methodName: "approve",
        params: [firstHighNetWorthAccount, "1"],
        signingCredential: {
          ethAccount: firstHighNetWorthAccount,
          secret: besuKeyPair.privateKey,
          type: Web3SigningCredentialType.PrivateKeyHex,
      ...
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20UnlockGasUsed.csv",
        "Gas (units)\n",
        "" +
          (response2.out.transactionReceipt.gasUsed +
            response.out.transactionReceipt.gasUsed),
      );
      expect(response2).toBeTruthy();
      expect(response2.success).toBeTruthy();
      log.info("Unlocked ERC20 " + i);
    }
  });
  ...
  it("Assing ERC20", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      writeToCSV(
        "/home/kubaya/Desktop/tests/ERC20AssignGasUsed.csv",
        "Gas (units)\n",
        "" + response.out.transactionReceipt.gasUsed,
      );
      expect(response).toBeDefined();
      log.info("Assigned ERC20 " + i);
    }
  });
  ...
  it("Should Unwrap a token", async () => {
    for (let i = 0; i < 50; i++) {
      ...
      const response2 = await besuBridge.unwrapAsset(tokenId);
