...
export class GetBalanceEndpointV1 implements IWebServiceEndpoint {
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      let result;
      if (req.query.chain as string == TransactRequestSourceChainAssetTypeEnum.Besu) {
        result = await this.options.infrastructure.getBesuBalance(req.query.user as string);
      } else {
        result = await this.options.infrastructure.getFabricBalance(req.query.user as string);
      }
      res.json(result);
    } catch (ex) {
