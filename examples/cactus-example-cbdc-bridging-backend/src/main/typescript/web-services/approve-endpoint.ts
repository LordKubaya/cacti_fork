...
import { ApproveRequest, TransactRequestSourceChainAssetTypeEnum } from "../generated/openapi/typescript-axios/api";
...
export class ApproveEndpointV1 implements IWebServiceEndpoint {
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      let result;
      if (reqBody.ledger.assetType === TransactRequestSourceChainAssetTypeEnum.Besu) {
        result = await this.options.infrastructure.approveNTokensBesu(reqBody.user, parseInt(reqBody.amount));
      } else {
        result = await this.options.infrastructure.approveNTokensFabric(reqBody.user, reqBody.amount);
      }
      res.json(result);
    } catch (ex) {
