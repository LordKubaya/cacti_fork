...
export class TransactEndpointV1 implements IWebServiceEndpoint {
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      const result = await this.options.infrastructure.bridgeTokens(reqBody.sender, reqBody.receiver, reqBody.sourceChain.assetType!, reqBody.receiverChain.assetType!, parseInt(reqBody.amount));
      res.json(result);
    } catch (ex) {
