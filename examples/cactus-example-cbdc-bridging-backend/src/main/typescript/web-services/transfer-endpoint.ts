...
import { TransactRequestSourceChainAssetTypeEnum, TransferRequest } from "../generated/openapi/typescript-axios/api";
...
export class TransferEndpointV1 implements IWebServiceEndpoint {
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      let result;
      if (reqBody.sourceChain.assetType === TransactRequestSourceChainAssetTypeEnum.Besu &&
        reqBody.receiverChain.assetType === TransactRequestSourceChainAssetTypeEnum.Fabric
      ) {
        result = await this.options.infrastructure.transferTokensBesu(reqBody.from, reqBody.to, parseInt(reqBody.amount));
      } else if (
        reqBody.sourceChain.assetType === TransactRequestSourceChainAssetTypeEnum.Fabric &&
        reqBody.receiverChain.assetType === TransactRequestSourceChainAssetTypeEnum.Besu
      ){
        result = await this.options.infrastructure.transferTokensFabric(reqBody.from, reqBody.to, reqBody.amount);
      }
      else {
