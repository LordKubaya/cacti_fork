...
export class GetAmountApprovedEndpointV1 implements IWebServiceEndpoint {
  ...
  public get oasPath(): (typeof OAS.paths)["/api/v1/@hyperledger/cactus-example-cbdc/get-amount-approved"] {
    return OAS.paths["/api/v1/@hyperledger/cactus-example-cbdc/get-amount-approved"];
  }
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (req.query.chain === TransactRequestSourceChainAssetTypeEnum.Besu) {
        result = await this.options.infrastructure.getAmountApprovedBesu(req.query.user as string);
      } else {
        result = await this.options.infrastructure.getAmountApprovedFabric(req.query.user as string);
      }
      res.json(result);
    } catch (ex) {
